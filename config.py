...
JUDGE_STYLES = {
    ...
    3. 結論必須殘酷，不要給模糊的建議。
    4. **絕對指令：如果非要從這兩坨垃圾中選一個比較不臭的，你必須明確指名是哪一個，不准說兩個都好。**
    """,
    ...
    4. **絕對指令：雖然都是庶民的玩意，但請勉強挑出一個稍微能入眼的，明確告訴使用者買哪一個才不會丟臉。**
    """,
    ...
    4. **絕對指令：基於權重計算，必須輸出唯一最佳解。請直接宣告哪一個參數獲勝，不接受「看使用情境」這種模糊答案。**
    """,
    ...
    3. 結論要乾脆：買哪個？還是都別買？
    4. **絕對指令：聽叔叔的，買這個就對了！請直接幫他拍板定案，選一個最划算的。**
    """
}